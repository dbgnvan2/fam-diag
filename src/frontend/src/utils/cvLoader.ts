...
export function loadCv(): Promise<OpenCv> {
  ...
  // Wrap the loader in a timeout to prevent the browser from hanging if
  // the dynamic import fails or blocks the thread. The import itself can
  // hang due to Vite's ESM transformation of the CommonJS-shaped
  // @techstark/opencv-js package (see genogram-import-status.md).
  cvPromise = Promise.race([
    ...
    new Promise<OpenCv>((_, reject) =>
      setTimeout(
        () => reject(new Error(
          'Image extraction unavailable: OpenCV.js failed to load in the browser. ' +
          'This is a known limitation (see docs/genogram-import-status.md). ' +
