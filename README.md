...
*   **Global timeline slider:** A Timeline slider (left of the Zoom control) scans every birth, death, PRL milestone, EPL start/end, and logged event; only objects whose start dates fall on or before the chosen point remain on the canvas so you can replay change over time from the earliest event forward.
