...
5.  **Selection + Context Menus:**
    ...
    *   Drag the Timeline slider (toolbar, left of Zoom) toward newer dates and confirm people, PRLs, and EPLs whose primary start dates are after the selected date remain hidden—only entries up to the chosen point stay visible. Slide back toward the start to show fewer items (only the earliest events), and advance gradually to watch new nodes appear.
